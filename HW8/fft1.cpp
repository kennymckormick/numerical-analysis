...
int main(){
    ...
    rep(i,0,lenb-1) B[rev(i)]=complex(1.0);
	C[rev(0)]=complex(4.0);
	C[rev(1)]=C[rev(1023)]=complex(-1.0);
	FFT(B,1);
	FFT(C,1);
	rep(i,0,1023) A[rev(i)]=B[i]/C[i];
	FFT(A,-1);
	rep(i,0,1023) printf("%lf %lf,",A[i].r,A[i].i);
}
